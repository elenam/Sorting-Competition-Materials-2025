...
public class Group7 {
	...
	// Stable least-significant-digit radix sort for equal-length binary strings.
	private static void radixSortBinary(String[] arr) {
		if (arr == null || arr.length <= 1) return;
		int len = arr[0].length();
		int n = arr.length;
		String[] aux = new String[n];

		// Process from least-significant bit (rightmost) to most-significant (leftmost)
		for (int pos = len - 1; pos >= 0; pos--) {
			int zeroCount = 0;

			for (int i = 0; i < n; i++) {
				if (arr[i].charAt(pos) == '0') zeroCount++;
			}
			...
			for (int i = 0; i < n; i++) {
				if (arr[i].charAt(pos) == '0') aux[zeroIndex++] = arr[i];
				else aux[oneIndex++] = arr[i];
			}
